...
重新整理会替换图谱和知识文档（含文档的手动编辑），操作前有提示；当前会话支持撤销最近一次成功整理；此后手动保存知识点会关闭这次撤销，以免丢失新修改。无法恢复的网络、凭据或存储异常会保留旧结果与已保存的提取检查点；处理期间素材或文档发生变化，也不会提交过期结果。Linux 桌面导出到 `~/Downloads`，同名文件自动编号。

### 导入与公式

- UTF-8 文本：`.txt`、`.md`、`.markdown`、`.tex`、`.latex`、`.csv`、`.json`。
- 多文件选择、拖入文档；长文本按完整段落与公式边界分批处理，不再使用旧的 1 MB / 100,000 字符导入上限。
...
1. **语义提取**：按完整 Markdown/LaTeX 块读取原文，公式、矩阵和公式环境不按行数或字符数拆开。模型提取中文概念名称及完整说明，一个原文块可以支撑多个概念，多个块也可共同支撑同一概念。程序按段落编号附上原文证据，通过校验的提取条目会逐批保存，供中断后复用。
2. **结构与概念分离**：标题、编号、目录、文件名、排版命令等只作为理解上下文；表格、列表中的实际知识照常提取。缺失或残缺的概念会在同一次任务中补处理。补处理携带有限的相邻原文，由模型判断是独立知识还是上下文，并保存分类理由与原文；不使用特定学科或笔记措辞的规则。同一概念在不同文件、不同语言中的重复定义会再做一次语义归并。
...
提取采用有限重试与工作队列，完整 JSON 条目可从截断响应中恢复，只补处理未覆盖内容。持续无效且不可再拆分的内容会明确标注“原文保留，未作 AI 提取”，不会把整次任务退回零结果或要求反复点击继续。主题目录格式异常会重试，结构中遗漏的知识点会补齐到图谱和文档。旧的素材总数、总字符数与知识点数量上限已移除，各模型请求仍按小批次执行。每次模型请求超时 180 秒；整个多阶段任务可能需要数分钟，并消耗多次 API 调用。所有阶段进度会显示在界面。纠错与补充遵循开关，但模型语义与事实质量仍应结合原文核对。
